...
import { designStyleDirectives } from "./design-style-directives";
import { generateRequestSchema, type DesignStyle } from "./generate-request-schema";
...
/**
 * Validates the request and starts a Gemini stream, returning a Web ReadableStream
 * of UTF-8 TSX text. Runs inside a Next.js Node runtime route. GEMINI_API_KEY stays
 * server-side. Mirrors the previous Express `streamPromptGenerateToResponse`.
 */
export async function createPromptGenerateStream(
  body: unknown,
    ...
    try {
      streamResponse = await ai.models.generateContentStream({
        ...
        contents: {
          ...
          parts: [
            {
              text: buildUserPrompt({ prompt, previousCode, systemInstruction }),
            },
          ],
        },
        config: {
  ...
  if (!streamResponse) {
    ...
    return {
      ok: false,
      status: isTransientCapacityError(lastError) ? 503 : 502,
      error: message,
    };
  }
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      try {
        ...
        // Stream already started: surface the error inline as a TSX comment.
